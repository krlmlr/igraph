...
static igraph_error_t igraph_i_layout_sugiyama_calculate_barycenters(const igraph_t* graph,
    ...
    for (i = 0; i < n; i++) {
        IGRAPH_CHECK(igraph_neighbors(graph, &neis, VECTOR(*layer_members)[i], direction, IGRAPH_NO_LOOPS, IGRAPH_MULTIPLE));
...
static igraph_error_t igraph_i_layout_sugiyama_place_nodes_horizontally(const igraph_t* graph,
    ...
    */
    ...
    for (i = 0; i < no_of_layers - 1; i++) {
        ...
        for (j = 0; j < n; j++) {
            IGRAPH_CHECK(igraph_neighbors(graph, &neis2, VECTOR(*vertices)[j], IGRAPH_OUT, IGRAPH_NO_LOOPS, IGRAPH_MULTIPLE));
...
static igraph_error_t igraph_i_layout_sugiyama_vertical_alignment(const igraph_t* graph,
    ...
    for (; i != i_limit; i += di) {
        ...
        for (; j != j_limit; j += dj) {
            ...
            IGRAPH_CHECK(igraph_neighbors(graph, &neis, vertex, neimode, IGRAPH_NO_LOOPS, IGRAPH_MULTIPLE));
